...
if [[ $(echo "$2") == "match" ]]
then
	if [[ "$1" =~ fcn\-train\-p[0-9]+\-gpu[0-9]+\-b[0-9]+\-e[0-9]+\/ ]] ; then
		var=$(echo "$1" | grep -E -o ".+/")
		...
		aws logs get-log-events --log-group-name /aws/sagemaker/TrainingJobs --log-stream-name $1 | python3 read-log-messages.py > logs/result-p$p-$gpu-$b-e$e.txt
	else
