...
def plot_accuracy_vs_time(data, chart_title, output_filename):
    ...
    for cs, cs_data in data.items():
        for bs, bs_data in cs_data.items():
            ...
            if not str(cs)+str(bs) in line_color:
                line_color[str(cs)+str(bs)] = idx_to_color(color_idx)
                color_idx += 1
            ax.plot(x_values,y_values,label="{} - {}".format(cs,bs),color=line_color[str(cs)+str(bs)])
...
def main():
    ...
    parser.add_argument("-i", "--ignore_first_iteration", action='store_true', help="Ignore first iteration when computing epoch times.")
    
    args = parser.parse_args()

    # Read results - one filename per experimental results

    accuracy_vs_time_dataset = read_results(args.filenames, args.ignore_first_iteration)
